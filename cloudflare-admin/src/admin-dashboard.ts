    ...
    <aside class="sidebar">
      ...
      <div class="sidebar-scroll">
        ...
        <nav class="category-nav" aria-label="Content categories">
          <button class="nav-item active" data-view="content" data-type=""><span class="nav-icon">⌂</span><span>Overview</span><b data-count="all">0</b></button>
          <button class="nav-item" data-view="content" data-type="post"><span class="nav-icon">✦</span><span>Posts</span><b data-count="post">0</b></button>
          <button class="nav-item" data-view="content" data-type="project"><span class="nav-icon">◇</span><span>Projects</span><b data-count="project">0</b></button>
          <button class="nav-item" data-view="content" data-type="blog"><span class="nav-icon">▤</span><span>Blog</span><b data-count="blog">0</b></button>
          <button class="nav-item" data-view="content" data-type="opinion"><span class="nav-icon">◌</span><span>Opinions</span><b data-count="opinion">0</b></button>
          <button class="nav-item" data-view="content" data-type="music"><span class="nav-icon">♫</span><span>Music</span><b data-count="music">0</b></button>
          <button class="nav-item" data-view="content" data-type="page"><span class="nav-icon">□</span><span>Pages</span><b data-count="page">0</b></button>
        </nav>
        <p class="nav-label system-label">System</p>
        <nav class="system-nav" aria-label="Admin settings">
          <button class="nav-item" data-view="audit"><span class="nav-icon">◉</span><span>Security log</span></button>
          <button class="nav-item" data-view="account"><span class="nav-icon">⚙</span><span>Account</span></button>
        </nav>
      </div>
...
.admin-shell{min-height:100vh;display:grid;grid-template-columns:258px minmax(0,1fr)}.sidebar{position:sticky;top:0;height:100vh;padding:26px 18px 18px;background:var(--dark);color:#fff;display:flex;flex-direction:column;overflow:hidden}.sidebar .brand{color:#fff;padding:0 10px}.sidebar .brand>span:last-child{color:#9c9cad}.sidebar-scroll{overflow:auto;scrollbar-width:none}.sidebar-scroll::-webkit-scrollbar{display:none}.nav-label{margin:48px 11px 10px;color:#777786;font-size:.62rem;font-weight:800;letter-spacing:.14em;text-transform:uppercase}.system-label{margin-top:28px}.category-nav,.system-nav{display:grid;gap:4px}.nav-item{width:100%;border:0;background:transparent;border-radius:11px;padding:10px 11px;text-align:left;color:#a9a9b7;display:grid;grid-template-columns:24px 1fr auto;gap:8px;align-items:center;font-size:.84rem;transition:.17s}.nav-icon{font-size:.96rem;text-align:center;color:#777786}.nav-item b{background:#2b2b36;color:#a9a9b7;border-radius:99px;padding:2px 7px;font-size:.66rem;min-width:24px;text-align:center}.nav-item:hover{background:#22222c;color:#fff}.nav-item.active{background:#302e51;color:#fff;font-weight:730}.nav-item.active .nav-icon{color:#a9a2ff}.nav-item.active b{background:#49447a;color:#fff}.sidebar-foot{margin-top:auto;border-top:1px solid #2b2b35;padding:17px 7px 0;display:grid;grid-template-columns:34px 1fr 34px;gap:9px;align-items:center}.avatar{width:34px;height:34px;border-radius:10px;background:linear-gradient(135deg,#7770ff,#5750d8);display:grid;place-items:center;font-weight:800}.owner{display:grid;min-width:0}.owner strong{font-size:.78rem;overflow:hidden;text-overflow:ellipsis}.owner small{font-size:.65rem;color:#7f7f8e}.signout{width:34px;height:34px;border:0;border-radius:9px;background:#24242e;color:#aaaab7;font-size:1rem}.signout:hover{color:#fff;background:#32323e}
...
@media(max-width:1180px){.table-head,.content-row{grid-template-columns:34px minmax(190px,1fr) 78px 95px minmax(220px,auto)}.table-head>span:nth-of-type(4),.date-cell{display:none}.editor-layout{grid-template-columns:minmax(0,1fr) 270px}}
...
function switchSection(button){state.view=button.dataset.view;state.type=state.view==='content'?(button.dataset.type||''):state.type;state.selected.clear();$$('.nav-item').forEach(item=>item.classList.toggle('active',item===button));$('#contentView').hidden=state.view!=='content';$('#auditView').hidden=state.view!=='audit';$('#accountView').hidden=state.view!=='account';$('#newBtn').hidden=state.view!=='content';const config=state.type?TYPES[state.type]:null;if(state.view==='content'){$('#viewKicker').textContent=config?'Website category':'Personal publishing system';$('#viewTitle').textContent=config?config.label:'Website overview';$('#viewDescription').textContent=config?'Create, edit, organize, and publish '+config.label.toLowerCase()+'.':'Manage everything visitors can see from one place.';$('#newTypeLabel').textContent=config?config.single:'content';render();}else if(state.view==='audit'){$('#viewKicker').textContent='Protection & accountability';$('#viewTitle').textContent='Security log';$('#viewDescription').textContent='Review access attempts and every recorded change.';loadAudit();}else{$('#viewKicker').textContent='Owner settings';$('#viewTitle').textContent='Account security';$('#viewDescription').textContent='Manage the credentials protecting this workspace.';}}
...
async function boot(){try{const session=await api('/api/admin/session');state.csrf=session.csrf;$('#adminName').textContent=session.username;await loadContent();}catch(error){console.error(error);}}boot();
`;
